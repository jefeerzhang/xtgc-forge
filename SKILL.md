...
Step 3 · 涌现主题       从 gap 出候选 → 用户选 1
       ↓ 🛑 Checkpoint #3(主题选择 + Grill 追问)
Step 4 · 提炼假设       DAG + 反事实 + 效应量 → 用户确认
       ↓ 🛑 Checkpoint #4(假设确认 + Grill 追问)
...
- **v0.2.3**(2026-08-10):**用户交互增强**。参考 `research-topic-selection` 加"启动说明" + "三问启动"(Phase 0 BLOCKING)。
- **v0.2.4**(2026-08-10):**Checkpoint + Grill 双增**。从 2 个 checkpoint 增至 5 个(文献确认 / 矩阵审阅 / 主题选择 / 假设确认 / 最终交付),每个 checkpoint 用 AskUserQuestion 逐项追问 + 推荐答案;Grill 机制细节化,主题选择和假设确认时逐项 grill。