...
**动作**(对选定主题):
1. Agent 扮演魔鬼代言人,给出 **2-3 条「最可能被审稿人拒的理由」**(识别策略 / 理论增量 / 数据可得 / 贡献重复,至少覆盖两种类型)
2. 每条理由给 **1 句回应(rebuttal)**:引用文献或 gap 依据,说明如何接住;接不住的,降级到备选
3. 写入 Step3b 的「## 对抗压测」小节

**半强校验**:`check_step.py --step 3b` 若发现 Step3b 含「对抗压测」标题,则强制该小节含「魔鬼代言」「最可能被拒」「回应」;未启用对抗则不拦(向后兼容)。**启用即做完整,不启用不强求。**

**与 Checkpoint #3 的关系**:对抗压测是用户点名后的「质检 + 解释」,不替代用户拍板;若回应接不住被拒理由,把降级建议带给用户,仍由用户决定是否换题。
